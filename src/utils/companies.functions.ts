...
import type { Company, AttractivenessScore } from './types'
...
export function calculateAttractivenessScore(c: Company): AttractivenessScore {
  ...
  let salaryScore = 50
  if (c.salary_vs_industry_pct !== null && c.salary_vs_industry_pct !== undefined) {
  ...
  const salaryDetail = c.salary_vs_industry_pct !== null && c.salary_vs_industry_pct !== undefined
    ? `同業${c.salary_vs_industry_pct > 0 ? '+' : ''}${c.salary_vs_industry_pct.toFixed(0)}%`
    : c.salary_median_k ? `中位數 ${(c.salary_median_k / 10).toFixed(0)} 萬` : '未揭露'

  // 2. Growth Momentum (20%)
  let growthScore = 50
  const revScore = c.revenue_yoy_pct !== null && c.revenue_yoy_pct !== undefined
    ? clamp(lerp(c.revenue_yoy_pct, -30, 30, 0, 100)) : 50
  const salGrowthScore = c.salary_median_change_pct !== null && c.salary_median_change_pct !== undefined
    ? clamp(lerp(c.salary_median_change_pct, -10, 20, 0, 100)) : 50
  growthScore = revScore * 0.7 + salGrowthScore * 0.3
  const growthParts: string[] = []
  if (c.revenue_yoy_pct !== null && c.revenue_yoy_pct !== undefined) growthParts.push(`營收${c.revenue_yoy_pct > 0 ? '+' : ''}${c.revenue_yoy_pct.toFixed(1)}%`)
  if (c.salary_median_change_pct !== null && c.salary_median_change_pct !== undefined) growthParts.push(`薪資${c.salary_median_change_pct > 0 ? '+' : ''}${c.salary_median_change_pct.toFixed(1)}%`)
  const growthDetail = growthParts.length > 0 ? growthParts.join('、') : '資料不足'
  ...
  const hiringDetail = totalJobs > 0
  ...
  if (c.eps !== null && c.eps !== undefined) {
    financeScore = c.eps > 0 ? clamp(lerp(c.eps, 0, 15, 40, 90)) : clamp(lerp(c.eps, -5, 0, 10, 40))
  }
  if (c.market_cap && c.market_cap > 1000) financeScore = Math.min(100, financeScore + 10)
  const financeDetail = c.eps !== null ? `EPS ${c.eps} 元` + (c.market_cap ? `、市值 ${c.market_cap >= 10000 ? (c.market_cap / 10000).toFixed(1) + ' 兆' : c.market_cap.toFixed(0) + ' 億'}` : '') : '未揭露'

  // 5. Risk Flags (10%, deduction)
  const flagCount = (c.flag_low_salary === 1 ? 1 : 0)
  ...
  const riskDetail = riskLabels.length > 0 ? riskLabels.join('、') : '無風險旗標'

  const overall = Math.round(
    salaryScore * 0.3 + growthScore * 0.2 + hiringScore * 0.2 + financeScore * 0.2 + riskScore * 0.1
  )
  ...
  return {
    ...
    grade,
    dimensions: [
      { name: '薪資競爭力', icon: '💰', score: Math.round(salaryScore), detail: salaryDetail },
      { name: '成長動能', icon: '📈', score: Math.round(growthScore), detail: growthDetail },
      { name: '擴編趨勢', icon: '🏢', score: Math.round(hiringScore), detail: hiringDetail },
      { name: '財務健康', icon: '⚖️', score: Math.round(financeScore), detail: financeDetail },
      { name: '風險旗標', icon: '🚩', score: Math.round(riskScore), detail: riskDetail },
    ],
  }
}
