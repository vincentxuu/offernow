...
export function CompanyCard({ company: c }: { company: Company }) {
  const salaryWan = c.salary_median_k
    ...
    >
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold ${gradeColor.bg} ${gradeColor.text}`}
            >
              {score.grade}
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <MetricBox
          ...
          sub={
            <div className="flex flex-wrap items-center gap-1">
              ...
              {vsIndustry !== null &&
                vsIndustry !== undefined &&
                Math.abs(vsIndustry) > 5 && (
                  <span
                    className={`text-[10px] font-medium ${vsIndustry > 0 ? 'text-[var(--green-positive)]' : 'text-[var(--red-negative)]'}`}
                  >
                    同業{vsIndustry > 0 ? '+' : ''}
        ...
        <MetricBox
          ...
          sub={
            c.revenue_yoy_pct !== null && c.revenue_yoy_pct !== undefined ? (
              <Badge
                value={c.revenue_yoy_pct > 999 ? 999 : c.revenue_yoy_pct}
                suffix=""
              />
            ) : undefined
          }
        ...
        <MetricBox
          ...
          sub="元/股"
        />
        <MetricBox label="市值" value={formatMarketCap(c.market_cap)} />
      </div>
      ...
      {c.ai_insight && (
        <div className="mt-2 rounded-lg bg-[var(--accent-soft)] px-3 py-2">
          ...
          <div className="text-xs text-[var(--text-heading)]">
            {c.ai_insight}
          </div>
        </div>
      )}
      ...
      {((c.job_count_104 ?? 0) > 0 || (c.job_count_linkedin ?? 0) > 0) && (
        <div className="mt-2 flex flex-wrap items-center gap-x-3 text-xs font-medium text-[var(--accent)]">
          {(c.job_count_104 ?? 0) > 0 && (
            <span>104: {c.job_count_104!.toLocaleString()} 缺</span>
          )}
          {(c.job_count_linkedin ?? 0) > 0 && (
            <span>LinkedIn: {c.job_count_linkedin!.toLocaleString()}+ 缺</span>
          )}
      ...
      {(c.flag_low_salary === 1 ||
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {c.flag_low_salary === 1 && (
            <span className="rounded bg-[var(--red-soft)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--red-negative)]">
              平均未達50萬
            </span>
          )}
          {c.flag_eps_high_salary_low === 1 && (
